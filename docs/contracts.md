...
`--cache` accepts `auto`, `only`, `refresh`, or `off`. Auto metadata freshness is 24 hours and an auto metadata hit is labelled `cached`. Artifact hits in both auto and only mode are labelled `not_checked`: their bytes and hashes are verified, but their upstream freshness is not rechecked. Only mode never performs network access. Artifact hits have no freshness TTL; their original retrieval timestamp and hash remain unchanged. Refresh atomically replaces mutable query/artifact indexes; content-addressed blobs remain create-once unless an explicit refresh repairs corruption.
